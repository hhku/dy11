...
def parse_video_info_from_url(url: str) -> VideoUrlInfo:
    ...
    # Extract ID from standard note URL: /note/number
    note_pattern = r'/note/(\d+)'
    match = re.search(note_pattern, url)
    if match:
        aweme_id = match.group(1)
        return VideoUrlInfo(aweme_id=aweme_id, url_type="normal")

    # Extract ID from share URLs resolved from short links: /share/video/number or /share/note/number
    share_pattern = r'/share/(?:video|note)/(\d+)'
    ...
    if "v.douyin.com" in url or url.startswith("http") and len(url) < 50 and "video" not in url and "note" not in url:
