...
class AsyncFileWriter:
    ...
    async def write_single_item_to_json(self, item: Dict, item_type: str):
        ...
        async with self.lock:
            ...
            if item_type == "comments" and item.get("comment_id"):
                comment_id = str(item.get("comment_id"))
                replaced = False
                for index, existing_item in enumerate(existing_data):
                    if not isinstance(existing_item, dict):
                        continue
                    if str(existing_item.get("comment_id")) == comment_id:
                        existing_data[index] = item
                        replaced = True
                        break
                if not replaced:
